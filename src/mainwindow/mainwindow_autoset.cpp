...
void MainWindow::autosetVrange() {
  ...
  if (activeAnalogs.count() + activeLogic.count() > 1) {
    ...
    for (int i = activeLogic.size() - 1; i >= 0; i--) {
      ...
      // Skupina logických kanálů má spodek na 0 a vršek je 3*počet kanálů
      // (bitů)
    ...
    for (int i = 0; i < activeAnalogs.size(); i++) {
      ...
      if (channelExpectedRanges[activeAnalogs.at(i)].unknown) {
        bool foundRange;
        range = ui->plot->graph(activeAnalogs.at(i))
                    ->data()
                    ->valueRange(foundRange);
        if (foundRange) {
          range.lower = ceilToNiceValue(range.lower) *
                        ui->plot->getChScale(activeAnalogs.at(i));
          range.upper = ceilToNiceValue(range.upper) *
                        ui->plot->getChScale(activeAnalogs.at(i));
        } else
          range = QCPRange(0, 0);
      } else
        range =
            QCPRange(channelExpectedRanges[activeAnalogs.at(i)].minimum,
                     channelExpectedRanges[activeAnalogs.at(i)].maximum * 1.1);
    ...
    if (recentOffset != 0) {
      ...
      ui->doubleSpinBoxRangeVerticalRange->setValue(
          ceilToMultipleOf(recentOffset, ui->plot->getVDiv()));
    ...
    if (!activeAnalogs.isEmpty()) {
      ...
      if (channelExpectedRanges[activeAnalogs.first()].unknown) {
        bool foundRange;
        range = ui->plot->graph(activeAnalogs.first())
                    ->data()
                    ->valueRange(foundRange);
        if (foundRange) {
          range.lower = ceilToNiceValue(range.lower) *
                        ui->plot->getChScale(activeAnalogs.first());
          range.upper = ceilToNiceValue(range.upper) *
                        ui->plot->getChScale(activeAnalogs.first());
        } else
          range = QCPRange(0, 0);
      } else
        range = QCPRange(channelExpectedRanges[activeAnalogs.first()].minimum,
                         channelExpectedRanges[activeAnalogs.first()].maximum);
      ...
      if (range.lower >= 0) {
        ...
        ui->doubleSpinBoxRangeVerticalRange->setValue(
            2 * MAX(std::abs(range.upper), std::abs(range.lower)));
      ...
      // Skupina logických kanálů má spodek na 0 a vršek je 3*počet pod-kanálů
      // (bitů)
      ui->doubleSpinBoxRangeVerticalRange->setValue(
          bitsUsed * 3 * ui->plot->getLogicScale(activeLogic.at(0)));
      ui->checkBoxVerticalValues->setChecked(false);
      on_pushButtonSetCenter_clicked();
      ui->comboBoxSelectedChannel->setCurrentIndex(
          LOGIC_GROUP_TO_CH_LIST_INDEX(activeLogic.at(0)));
    }
  }
...
void MainWindow::autosetHrange() {
  if (ui->plot->getLastDataTypeWasPoint()) {
    on_pushButtonModeRolling_clicked();
    if (dataUpdates > 10)
...
void MainWindow::on_pushButtonResetChannels_clicked() {
  ...
  for (int i = 0; i < ANALOG_COUNT + MATH_COUNT; i++)
    if (ui->plot->isChUsed(i) && ui->plot->isChVisible(i)) {
      ...
      if (channelExpectedRanges[i].unknown) {
        ...
        chRange = ui->plot->graph(i)->getValueRange(
            zbytrecnaPromena, QCP::sdBoth, ui->plot->xAxis->range());
      } else {
  ...
  if (valueRange.lower >= 0) {
    ...
    ui->doubleSpinBoxRangeVerticalRange->setValue(
        2 * MAX(std::abs(valueRange.upper), std::abs(valueRange.lower)));
    on_pushButtonSetCenter_clicked();
  }
  ui->checkBoxVerticalValues->setChecked(true);
