...
void ManualInputDialog::initRollingTable() {
  ...
  QPushButton* addRowButton = new QPushButton(QIcon(":/images/icons/plus.png"),
                                              "", ui->tableWidgetRollingSetup);
  QPushButton* removeRowButton = new QPushButton(
      QIcon(":/images/icons/minus.png"), "", ui->tableWidgetRollingSetup);
  ...
  ui->tableWidgetRollingSetup->setItem(0, 0,
                                       new QTableWidgetItem("5*sin(2*Pi*t)"));
  ui->tableWidgetRollingSetup->setItem(
      1, 0, new QTableWidgetItem("2*cos(2*Pi*t) + 0.1*random()*sin(100*t)"));
}

ManualInputDialog::ManualInputDialog(QWidget* parent)
    : QDialog(parent), ui(new Ui::ManualInputDialog) {
  ui->setupUi(this);
  ...
  connect(&rollingTimer, &QTimer::timeout, this,
          &ManualInputDialog::rollingDataTimerRoutine);
...
void ManualInputDialog::on_tableWidgetRollingSetup_cellChanged(int row,
                                                               int column) {
  if (column == 0) {
    auto txt = ui->tableWidgetRollingSetup->item(row, column)->text();
    bool ok =
        rollingChannelEvaluators.at(row)->setExpression(rollingEngine, txt);
    QTableWidgetItem* item = new QTableWidgetItem(ok              ? "OK"
                                                  : txt.isEmpty() ? "Empty"
                                                                  : "Error");
