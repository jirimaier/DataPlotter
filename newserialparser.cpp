...
void NewSerialParser::fatalError(QString header, QByteArray &message) {
  ...
  if (!buffer.isEmpty()) {
    if (buffer.contains("$$"))
      ...
      buffer.clear();
    if (!pendingDataBuffer.isEmpty()) pendingDataBuffer.clear();
    if (!pendingPointBuffer.isEmpty()) pendingPointBuffer.clear();
    resetChHeader();
  }
...
void NewSerialParser::sendMessageIfAllowed(QString header, QByteArray &message, MessageLevel::enumerator type) {
  if ((int)debugLevel >= (int)type) emit sendMessage(header, message, type, target);
}

void NewSerialParser::sendMessageIfAllowed(QString header, QString &message, MessageLevel::enumerator type) {
  if ((int)debugLevel >= (int)type) emit sendMessage(header, message.toUtf8(), type, target);
}
...
void NewSerialParser::parse(QByteArray newData) {
  ...
  while (!buffer.isEmpty()) {
    if (buffer.right(1) == "$") break;
    if (buffer.length() >= 3)
      if (buffer.left(2) == "$$") {
        if (currentMode == DataMode::info || currentMode == DataMode::warning) emit sendDeviceMessage("", false, true);  // Pokud byl předchozí režim výpis zprávy, ohlásí její konec
    ...
    if (currentMode == DataMode::unknown) {
      ...
      if (!dropped.isEmpty()) sendMessageIfAllowed(tr("Unknown"), dropped, MessageLevel::info);
      ...
      if (!channelHeaderOK) {
        ...
        if (result == complete) {
          if (pendingPointBuffer.length() == 3 || pendingPointBuffer.length() == 6) {
            ...
            if (channelNumber > ANALOG_COUNT || channelNumber == 0) isok = false;
            if (!isok) {
      ...
      if (channelHeaderOK) {
        ...
        if (result == notProperlyEnded) {
          ...
          if (buffer.contains(';') && channel.contains(';'))
            ...
            if (channel.contains(';')) semicolumPositionMessage = tr("There is semicolum %1 bytes before end.").arg(channel.length() - channel.lastIndexOf(';')).toUtf8();
            if (buffer.contains(';')) semicolumPositionMessage = tr("There is semicolum %1 bytes after end.").arg(buffer.indexOf(';')).toUtf8();
          }
...
NewSerialParser::readResult NewSerialParser::bufferPullPoint(QByteArrayList &result) {
  while (!buffer.isEmpty()) {
    ...
    if (isdigit(buffer.at(0)) || buffer.at(0) == '-' || buffer.at(0) == ',') {
      ...
      if (ending == none) return incomplete;
    ...
    // Binární data
      ...
      if (buffer.length() < 2 + bytes) return incomplete;
      ...
      if (buffer.length() >= 2)
        if (buffer.left(2) == "$$") return notProperlyEnded;
      continue;
    }
  }
  return incomplete;
...
NewSerialParser::readResult NewSerialParser::bufferPullChannel(QByteArray &result) {
  ...
  if (channelBits == -1) channelBits = 8 * bytesPerValue;
  if (!isok) {
    fatalError(tr("invalid type"), type);
    return incomplete;
  }
  if ((uint32_t)buffer.length() < channelLength * bytesPerValue + 3) return incomplete;
  result = buffer.left(channelLength * bytesPerValue + 2);
  buffer.remove(0, channelLength * bytesPerValue + 2);
  if (buffer.at(0) != ';') return notProperlyEnded;
  buffer.remove(0, 1);
  return complete;
}
