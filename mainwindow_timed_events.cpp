...
void MainWindow::comRefresh() {
    ...
    for (QSerialPortInfo port : portList) {
        if(!newPorts.contains(port)) {
            auto toremove = ui->listWidgetCom->findItems(port.portName() + " - " + port.description(),Qt::MatchExactly);
    ...
    if(change) {
        ...
        {
            auto candidates = ui->listWidgetCom->findItems(attemptReconnectPort,Qt::MatchExactly);
