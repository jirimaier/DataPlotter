...
MyPeakPlot::MyPeakPlot(QWidget* parent) : MyPlot(parent) {
  ...
  addGraph(xAxis, yAxis); addGraph(xAxis, yAxis);
...
void MyPeakPlot::setInfoText() {
  infoText->setText(tr("Enable FFT and zoom-in on peak that you want to track"));
}
...
void MyPeakPlot::setAutoSize(bool en) {
  autoSize = en;
...
void MyPeakPlot::autoset() {
  ...
  QCPRange yRange1 = graph(0)->data()->valueRange(foundrange1, QCP::sdBoth, xAxis->range());
  QCPRange yRange2 = graph(1)->data()->valueRange(foundrange2, QCP::sdBoth, xAxis->range());
  if (foundrange1 && foundrange2)
    yAxis->setRange(QCPRange(floorToNiceValue(MIN(yRange1.lower * 0.9, yRange2.lower * 0.9)), ceilToNiceValue(MAX(yRange1.upper * 1.1, yRange2.upper * 1.1))));
  else if (foundrange1)
    yAxis->setRange(floorToNiceValue(yRange1.lower * 0.9), ceilToNiceValue(yRange1.upper * 1.1));
  else if (foundrange2)
    yAxis->setRange(floorToNiceValue(yRange2.lower * 0.9), ceilToNiceValue(yRange2.upper * 1.1));
  else
  ...
  if ((xMax - xMin) > timeLength)
    xMin = xMax - timeLength;
