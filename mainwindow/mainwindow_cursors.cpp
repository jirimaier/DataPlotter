...
void MainWindow::updateCursorRange() {
  if (ui->horizontalSliderTimeCur1->isEnabled()) {
    ...
    if (IS_ANALOG_OR_MATH(ch)) {
      ...
      ui->doubleSpinBoxXCur1->setSingleStep(ui->plot->xAxis->range().size() /
                                            100);
      ...
      ui->doubleSpinBoxXCur1->setSingleStep(ui->plotFFT->xAxis->range().size() /
                                            100);
    } else {
      range = ui->plot->getChVisibleSamplesRange(
          getLogicChannelID(CH_LIST_INDEX_TO_LOGIC_GROUP(ch), 0));
      empty =
          ui->plot
              ->graph(getLogicChannelID(CH_LIST_INDEX_TO_LOGIC_GROUP(ch), 0))
              ->data()
              ->isEmpty();
      ui->doubleSpinBoxXCur1->setSingleStep(ui->plot->xAxis->range().size() /
                                            100);
  ...
  if (ui->horizontalSliderTimeCur2->isEnabled()) {
    ...
    if (IS_ANALOG_OR_MATH(ch)) {
      range = ui->plot->getChVisibleSamplesRange(ch);
      empty = ui->plot->graph(ch)->data()->isEmpty();
      ui->doubleSpinBoxXCur2->setSingleStep(ui->plot->xAxis->range().size() /
                                            100);
    } else if (IS_FFT_INDEX(ch)) {
      range = ui->plotFFT->getVisibleSamplesRange(INDEX_TO_FFT_CHID(ch));
      empty = ui->plotFFT->graph(INDEX_TO_FFT_CHID(ch))->data()->isEmpty();
      ui->doubleSpinBoxXCur2->setSingleStep(ui->plotFFT->xAxis->range().size() /
                                            100);
    } else {
      range = ui->plot->getChVisibleSamplesRange(
          getLogicChannelID(CH_LIST_INDEX_TO_LOGIC_GROUP(ch), 0));
...
void MainWindow::updateCursors() {
  ...
  if (ui->checkBoxCur1Visible->checkState() ==
      Qt::CheckState::PartiallyChecked) {
    updateCursor(
        Cursor1, ch1, ui->horizontalSliderTimeCur1->getRealValue(), time1,
        value1, timeStr, valueStr,
        ui->checkBoxYCur1->checkState() == Qt::CheckState::PartiallyChecked);
  ...
  if (ui->checkBoxCur2Visible->checkState() ==
      Qt::CheckState::PartiallyChecked) {
    updateCursor(
        Cursor2, ch2, ui->horizontalSliderTimeCur2->getRealValue(), time2,
        value2, timeStr, valueStr,
        ui->checkBoxYCur2->checkState() == Qt::CheckState::PartiallyChecked);
...
void MainWindow::updateCursorMeasurementsText() {
  ...
  int ch1 = ui->comboBoxCursor1Channel->currentIndex();
  int ch2 = ui->comboBoxCursor2Channel->currentIndex();
  if (ui->checkBoxCur1Visible->checkState() != Qt::CheckState::Unchecked &&
      ui->checkBoxCur2Visible->checkState() != Qt::CheckState::Unchecked) {
    double time1 = IS_FFT_INDEX(ch1)
                       ? ui->plotFFT->getTimeCursorPosition(Cursor1)
                       : ui->plot->getTimeCursorPosition(Cursor1);
    double time2 = IS_FFT_INDEX(ch2)
                       ? ui->plotFFT->getTimeCursorPosition(Cursor2)
                       : ui->plot->getTimeCursorPosition(Cursor2);
    double value1 = IS_FFT_INDEX(ch1)
                        ? ui->plotFFT->getValueCursorPosition(Cursor1)
                        : ui->plot->getValueCursorPosition(Cursor1);
    double value2 = IS_FFT_INDEX(ch2)
                        ? ui->plotFFT->getValueCursorPosition(Cursor2)
                        : ui->plot->getValueCursorPosition(Cursor2);
    double dt = time2 - time1;
    double dy = value2 - value1;
    // dB FFT
    if (IS_FFT_INDEX(ch1) && IS_FFT_INDEX(ch2) &&
        ui->comboBoxFFTType->currentIndex() != FFTType::spectrum) {
      if (freqUseUnits)
        ui->labelCurDeltaTime->setText(floatToNiceString(dt, 4, true, false) +
                                       ui->plotFFT->getXUnit());
      else
        ui->labelCurDeltaTime->setText(QString::number(dt, 'g', 3));
      ui->labelCurDeltaValue->setText(
          QString::number(dy, 'f', 3).rightJustified(7) + " dB");
    }

    // linear FFT
    else if (IS_FFT_INDEX(ch1) && IS_FFT_INDEX(ch2) &&
             ui->comboBoxFFTType->currentIndex() == FFTType::spectrum) {
      if (freqUseUnits)
        ui->labelCurDeltaTime->setText(floatToNiceString(dt, 4, true, false) +
                                       ui->plotFFT->getXUnit());
      ...
      ui->labelCurRatio->setText(
          floatToNiceString(value1 / value2, 4, true, true));
    }

    // Time
    else if (IS_ANALOG_OR_MATH_OR_LOGIC(ch1) &&
             IS_ANALOG_OR_MATH_OR_LOGIC(ch2)) {
      if (IS_ANALOG_OR_MATH(ch1) && IS_ANALOG_OR_MATH(ch2)) {
        // Both analog
        if (timeUseUnits)
          ui->labelCurDeltaTime->setText(floatToNiceString(dt, 4, true, true) +
                                         ui->plot->getXUnit());
        else
          ui->labelCurDeltaTime->setText(QString::number(dt, 'g', 3));

        if (ui->plot->getValueCursorVisible(Cursor1) ||
            ui->plot->getValueCursorVisible(Cursor2)) {
          QString YUnit = ui->plot->getYUnit();
          if (valuesUseUnits) {
            ui->labelCurDeltaValue->setText(
                floatToNiceString(dy, 4, true, true) + YUnit);
            ui->labelCurRatio->setText(
                floatToNiceString(value1 / value2, 4, true, true) + YUnit +
                "/" + YUnit);

            QString slope = floatToNiceString(dy / dt, 4, true, true);
            if (QString(" mnpfa" + QString::fromUtf8("\xc2\xb5"))
                    .contains(slope.at(slope.length() - 1)) ||
                ui->plot->getXUnit().isEmpty()) {
              slope =
                  slope + YUnit + "/" +
                  (ui->plot->getXUnit().isEmpty() ? "1" : ui->plot->getXUnit());
            } else {
            ...
            ui->labelCurDeltaValue->setText(QString::number(dy, 'g', 3) +
                                            YUnit);
            ui->labelCurRatio->setText(
                QString::number(value1 / value2, 'g', 3) +
                (YUnit.isEmpty() ? "" : (YUnit + "/" + YUnit)));
            ui->labelCurSlope->setText(
                QString::number(dy / dt, 'g', 3) +
                (YUnit.isEmpty() && ui->plot->getXUnit().isEmpty()
                     ? ""
                     : (YUnit + "/" + ui->plot->getXUnit())));
          }
        }

        if (freqUseUnits)
          ui->labelCurFreq->setText(
              floatToNiceString(std::abs(1.0 / dt), 4, true, true) +
              ui->plotFFT->getXUnit());
        else
          ui->labelCurFreq->setText(
              QString::number(std::abs(1.0 / dt), 'g', 3));
        ...
        if (freqUseUnits)
          ui->labelCurFreq->setText(
              floatToNiceString(std::abs(1.0 / dt), 4, true, false) +
              ui->plotFFT->getXUnit());
        else
          ui->labelCurFreq->setText(
              QString::number(std::abs(1.0 / dt), 'g', 3));
...
void MainWindow::updateCursor(Cursors::enumCursors cursor,
                              int selectedChannel,
                              unsigned int sample,
                              double& time,
                              double& value,
  ...
  if (IS_ANALOG_OR_MATH(selectedChannel)) {
    ...
    if (timeUseUnits)
      timeStr = QString(floatToNiceString(time, 5, true, false) +
                        ui->plot->getXUnit())
                    .toUtf8();
    else
    ...
    if (valuesUseUnits)
      valueStr =
          QString(floatToNiceString(value, 5, true, false) + unit).toUtf8();
    else
      valueStr = QString(QString::number(value, 'g', 5) + unit).toUtf8();

    if (useValueCursor) {
      ui->plot->updateValueCursor(cursor, value, valueStr,
                                  ui->plot->getAnalogAxis(selectedChannel));
    }
    ui->plot->updateTimeCursor(cursor, time, timeStr, selectedChannel);

  } else if (IS_FFT_INDEX(selectedChannel)) {
    // FFT
    double freq = ui->plotFFT->graph(INDEX_TO_FFT_CHID(selectedChannel))
                      ->data()
                      ->at(sample)
                      ->key;
    value = ui->plotFFT->graph(INDEX_TO_FFT_CHID(selectedChannel))
                ->data()
                ->at(sample)
                ->value;

    if (freqUseUnits)
      timeStr = QString(floatToNiceString(freq, 5, true, false) +
                        ui->plotFFT->getXUnit())
                    .toUtf8();
    else
      timeStr = QString(QString::number(freq, 'g', 5)).toUtf8();

    valueStr = (QString::number(value, 'f', 3).rightJustified(8) + " " +
                ui->plotFFT->getYUnit())
                   .toUtf8();
    time = freq;
    if (useValueCursor) {
      ui->plotFFT->updateValueCursor(cursor, value, valueStr,
                                     ui->plotFFT->yAxis);
    }
    ui->plotFFT->updateTimeCursor(cursor, freq, timeStr,
                                  INDEX_TO_FFT_CHID(selectedChannel));
    ...
    time =
        ui->plot->graph(getLogicChannelID(group, 0))->data()->at(sample)->key;
    ...
    valueStr = "0x" + QString::number((uint32_t)value, 16)
                          .toUpper()
                          .rightJustified(ceil(bitsUsed / 8.0) * 2, '0')
                          .toLocal8Bit();
    valueStr.append("\n" + bits);
    ui->plot->updateTimeCursor(cursor, time, valueStr + "\n" + timeStr,
                               getLogicChannelID(group, 0));
  }
}
...
void MainWindow::on_checkBoxCur1Visible_stateChanged(int arg1) {
  ...
  if (!IS_ANALOG_OR_MATH(selectedChannel) &&
      selectedChannel != CURSOR_ABSOLUTE && arg1 == Qt::CheckState::Checked) {
  ...
  if (ui->checkBoxYCur1->checkState() == Qt::PartiallyChecked &&
      arg1 == Qt::Unchecked)
  ...
  if (arg1 == Qt::CheckState::Checked) {
    ...
    if (IS_FFT_INDEX(selectedChannel))
      ui->doubleSpinBoxXCur1->setValue(
          ui->plotFFT->getTimeCursorPosition(Cursor1));
...
void MainWindow::on_checkBoxCur2Visible_stateChanged(int arg1) {
  ...
  if (ui->checkBoxYCur2->checkState() == Qt::PartiallyChecked &&
      arg1 == Qt::Unchecked)
...
void MainWindow::on_comboBoxCursor1Channel_currentIndexChanged(int index) {
  ui->horizontalSliderTimeCur1->setEnabled(
      ui->checkBoxCur1Visible->checkState() != Qt::Checked &&
      index != CURSOR_ABSOLUTE);

  if (index == CURSOR_ABSOLUTE &&
      ui->checkBoxCur1Visible->checkState() == Qt::PartiallyChecked)
    ui->checkBoxCur1Visible->setCheckState(Qt::CheckState::Checked);

  setCursorsVisibility(
      Cursor1, index,
      ui->checkBoxCur1Visible->checkState() != Qt::CheckState::Unchecked,
      ui->checkBoxYCur1->checkState());
...
void MainWindow::on_comboBoxCursor2Channel_currentIndexChanged(int index) {
  ui->horizontalSliderTimeCur2->setEnabled(
      ui->checkBoxCur2Visible->checkState() != Qt::Checked &&
      index != CURSOR_ABSOLUTE);

  if (index == CURSOR_ABSOLUTE &&
      ui->checkBoxCur2Visible->checkState() == Qt::PartiallyChecked)
    ui->checkBoxCur2Visible->setCheckState(Qt::CheckState::Checked);
...
void MainWindow::timeCursorMovedByMouse(Cursors::enumCursors cursor,
                                        int sample,
                                        double value) {
  if (cursor == Cursor1) {
...
void MainWindow::cursorSetByMouse(int chid,
                                  Cursors::enumCursors cursor,
                                  int sample) {
  if (cursor == Cursor1) {
...
void MainWindow::plotRecomendedAxisTypeChanged(
...
void MainWindow::on_checkBoxYCur1_stateChanged(int arg1) {
  ...
  if ((ui->checkBoxCur1Visible->checkState() != Qt::PartiallyChecked) &&
      ui->checkBoxYCur1->checkState() == Qt::PartiallyChecked) {
  ...
  setCursorsVisibility(Cursor1, selectedChannel,
                       ui->checkBoxCur1Visible->checkState(), arg1);
  if (arg1 == Qt::CheckState::Checked) {
    if (IS_FFT_INDEX(selectedChannel))
      ui->doubleSpinBoxYCur1->setValue(
          ui->plotFFT->getValueCursorPosition(Cursor1));
    else if (IS_ANALOG_OR_MATH(selectedChannel) ||
             selectedChannel == CURSOR_ABSOLUTE)
      ui->doubleSpinBoxYCur1->setValue(
...
void MainWindow::on_checkBoxYCur2_stateChanged(int arg1) {
  ...
  if (ui->checkBoxCur2Visible->checkState() != Qt::PartiallyChecked &&
      ui->checkBoxYCur2->checkState() == Qt::PartiallyChecked) {
  ...
  setCursorsVisibility(Cursor2, selectedChannel,
                       ui->checkBoxCur2Visible->checkState(), arg1);
  if (arg1 == Qt::CheckState::Checked) {
    if (IS_FFT_INDEX(selectedChannel))
      ui->doubleSpinBoxYCur2->setValue(
          ui->plotFFT->getValueCursorPosition(Cursor2));
    else if (IS_ANALOG_OR_MATH(selectedChannel) ||
             selectedChannel == CURSOR_ABSOLUTE)
      ui->doubleSpinBoxYCur2->setValue(
...
void MainWindow::setCursorsVisibility(Cursors::enumCursors cursor,
                                      int graph,
                                      int timeCurState,
                                      int valueCurState) {
...
void MainWindow::on_doubleSpinBoxYCur1_valueChanged(double arg1) {
  ...
  if (IS_FFT_INDEX(chID))
    ui->plotFFT->updateValueCursor(
        Cursor1, arg1, ui->doubleSpinBoxYCur1->text(), ui->plotFFT->yAxis);
  else
    ui->plot->updateValueCursor(Cursor1, arg1, ui->doubleSpinBoxYCur1->text(),
                                chID == CURSOR_ABSOLUTE
                                    ? ui->plot->yAxis
                                    : ui->plot->getAnalogAxis(chID));
  updateCursors();
}
...
void MainWindow::on_doubleSpinBoxYCur2_valueChanged(double arg1) {
  ...
  if (IS_FFT_INDEX(chID))
    ui->plotFFT->updateValueCursor(
        Cursor2, arg1, ui->doubleSpinBoxYCur2->text(), ui->plotFFT->yAxis);
  else
    ui->plot->updateValueCursor(Cursor2, arg1, ui->doubleSpinBoxYCur2->text(),
                                chID == CURSOR_ABSOLUTE
                                    ? ui->plot->yAxis
                                    : ui->plot->getAnalogAxis(chID));
  updateCursors();
...
void MainWindow::on_doubleSpinBoxXCur1_valueChanged(double arg1) {
  int chID = ui->comboBoxCursor1Channel->currentIndex();
  if (IS_FFT_INDEX(chID))
    ui->plotFFT->updateTimeCursor(Cursor1, arg1, ui->doubleSpinBoxXCur1->text(),
                                  INDEX_TO_FFT_CHID(chID));
  else if (chID == CURSOR_ABSOLUTE)
    ui->plot->updateTimeCursor(Cursor1, arg1, ui->doubleSpinBoxXCur1->text(),
                               -1);
  else
    ui->plot->updateTimeCursor(Cursor1, arg1, ui->doubleSpinBoxXCur1->text(),
                               chID);
...
void MainWindow::on_doubleSpinBoxXCur2_valueChanged(double arg1) {
  int chID = ui->comboBoxCursor2Channel->currentIndex();
  if (IS_FFT_INDEX(chID))
    ui->plotFFT->updateTimeCursor(Cursor2, arg1, ui->doubleSpinBoxXCur2->text(),
                                  INDEX_TO_FFT_CHID(chID));
  else if (chID == CURSOR_ABSOLUTE)
    ui->plot->updateTimeCursor(Cursor2, arg1, ui->doubleSpinBoxXCur2->text(),
                               -1);
  else
    ui->plot->updateTimeCursor(Cursor2, arg1, ui->doubleSpinBoxXCur2->text(),
                               chID);
